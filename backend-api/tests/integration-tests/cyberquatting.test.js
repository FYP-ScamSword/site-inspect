...
describe("Link Inspection Cybersquatting", () => {
  beforeAll(async () => {
    await db.mongoose
      ...
      .connect("mongodb://mongodb:27017/test", {
        ...
        db.cybersquat_known_sites.insertMany(knownSites);
      })
      .catch((err) => {
  ...
  it("Should detect levelsquatting or combosquatting", async () => {
    const records = await db.cybersquat_known_sites.find({});
  ...
  it("Should detect typosquatting or bitsquatting", async () => {
    const records = await db.cybersquat_known_sites.find({});
